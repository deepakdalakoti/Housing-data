...
class Mortgage:
    ...
    def get_monthly_payments(self):
        return np.ceil(
            self.interest * self.principal / (1 - (1 + self.interest) ** (-self.months))
        )

    def get_monthly_interest_payment(self):
        return np.ceil(self.interest * self.principal)

    def get_total_interest_paid(self, years, extra_payments=0):
        monthly_payments = self.get_monthly_payments() + extra_payments
        periods = years * 12
        return (self.principal * self.interest - monthly_payments) * (
    ...
    def get_stats_current_val(self, years, inflation, extra_payments=0):
        ...
        for i in range(years * 12):
            ...
            current_principal = (
                current_principal + (monthly - interest) / (1 + rate) ** i
            )
